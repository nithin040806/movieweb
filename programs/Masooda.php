...
$_SESSION['movieid']=6;
$_SESSION['pageurl']="http://localhost/project/programs/Masooda.php";
$user_data=check_login($con);
$a=$user_data['userid'] ;
$sql3="SELECT * from watchlist where movieid=6 AND userid=$a";
$result=mysqli_query($con,$sql3);
$watch=mysqli_fetch_assoc($result);
$sql5="SELECT * from favourite where movieid=6 AND userid=$a";
...
<?php 
  ...
  $sql="SELECT * FROM movies WHERE movieid=6";
  $car=$conn->query($sql);
  $row=mysqli_fetch_assoc($car);
  $sql4="SELECT m.postercardurl,m.pageurl FROM movies m  RIGHT JOIN languagebridge lb on m.movieid=lb.movieid WHERE lb.languageid=1 ORDER BY `m`.`postercardurl` DESC";
